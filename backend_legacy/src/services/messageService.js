...
// const { getIO } = require('../socket'); // Removido para migração Supabase
...
const createMessage = async (conversationId, senderId, content, isSystemMessage = false) => {
  ...
  // 4. Emissão via Supabase Realtime (refatorado)
  const { emitToRoom, notifyUser } = require('./realtimeService');

  // 4a. Emite para a sala da conversa (chat ao vivo)
  // Canal: conversationId
  try {
    await emitToRoom(conversationId, 'receive_message', newMessage);
  } catch (e) {
    console.error('Erro ao emitir receive_message supabase:', e);
  }

  // 4b. Emite notificação para a sala pessoal do destinatário
  if (recipientId) {
    try {
      await notifyUser(recipientId, 'new_message_notification', {
        conversationId: conversationId,
        message: newMessage,
        isSystemMessage: isSystemMessage
      });
      console.log(`Notificação Supabase enviada para user:${recipientId}`);
    } catch (e) {
      console.error('Erro ao emitir new_message_notification supabase:', e);
    }
  }
