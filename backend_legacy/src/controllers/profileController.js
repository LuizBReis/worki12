...
const { uploadFile, deleteFile } = require('../services/storageService'); // Importar serviço de storage
...
const updateMyAvatar = async (req, res) => {
  try {
    ...
    const publicUrl = await uploadFile(req.file, 'avatars', userId);

    // 3. Chama o service para salvar a URL no banco
    const updatedUser = await userService.updateUserAvatar(userId, publicUrl);
...
const deleteMyAvatar = async (req, res) => {
  try {
    ...
    // Opcional: Recuperar URL antiga para deletar do storage se necessário
    // const user = await userService.getUserById(userId);
    // if (user.avatarUrl) await deleteFile(user.avatarUrl, 'avatars');
...
const updateMyVideo = async (req, res) => {
  try {
    ...
    const publicUrl = await uploadFile(req.file, 'videos', userId);

    const updatedUser = await userService.updateUserVideo(userId, publicUrl);
    delete updatedUser.password;
    res.status(200).json(updatedUser);
  } catch (error) {
