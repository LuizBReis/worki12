-- Migration: Auto-create wallet for workers when needed (SEC-07)
-- Problem: if worker has no wallet when escrow auto-release triggers,
-- the escrow gets stuck forever with only a WARNING logged.
-- Solution: create wallet automatically in the auto_release trigger.
